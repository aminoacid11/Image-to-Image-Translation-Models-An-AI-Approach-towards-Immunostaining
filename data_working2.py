...
#         class_mode=None,
#         color_mode='rgb',  # Use 'rgb' color mode for 3-channel RGB images
...
def trainGenerator(batch_size,train_path,image_folder,mask_folder,aug_dict,target_size = (256,256),seed = 1):
    ...
    while True:
        ...
        img = img / 255.
        mask = mask / 255.
        img = img.reshape((batch_size,) + target_size + (3,))
        # mask = mask.reshape((batch_size,) + target_size + (1,))
        mask = mask.reshape((batch_size,) + target_size + (3,))
        yield (img,mask)
...
# def testGenerator(test_path, num_image=30, target_size=(256, 256), as_gray=False):
#     for i in range(num_image):
#         img = io.imread(os.path.join(test_path, "%d.png" % i))  # Read colored image
#         img = img / 255.
#         img = trans.resize(img, target_size)
#         img = img.reshape((1,) + target_size + (3,))
#         yield img
